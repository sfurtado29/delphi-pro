...
@router.get("/propensity")
def get_propensity_dashboard(
    ...
    try:
        ...
        results.sort(
            key=lambda x: float(x.get("propensity_score", 0) or 0),
            reverse=True
        )
        ...
        logging.error(f"Error fetching Propensity dashboard data: {e}")
        raise HTTPException(status_code=500, detail="Failed to fetch Propensity dashboard data")
